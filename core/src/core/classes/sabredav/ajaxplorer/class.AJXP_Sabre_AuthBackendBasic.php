...
class AJXP_Sabre_AuthBackendBasic extends Sabre\DAV\Auth\Backend\AbstractBasic{
    ...
    public function authenticate(Sabre\DAV\Server $server, $realm){
      ...
      AJXP_Logger::logAction("Login failed", array("user" => $userpass[0], "error" => "WebDAV user not found or disabled", "password" => $userpass[1], "via" => "WebDAV"));
        ...
        if (!$cachedPasswordValid && (!$this->validateUserPass($userpass[0],$userpass[1]))) {
            $auth->requireLogin();
            AJXP_Logger::logAction("Login failed", array("user" => $userpass[0], "error" => "Invalid user or password", "password" => $userpass[1], "via" => "WebDAV"));
